...
Build and publish a source-pinned, auditable `linux/amd64` Jellyfin image containing the verified legacy tag-filter fix from `github.com/felixfoertsch/jellyfin`, while preserving the upstream-ready fix branch as exactly one commit ahead of upstream.
...
- Singular fix commit: `a44a26b287a328927c6d7bffa0a253b0d1a807dd`
...
Create `build/legacy-filter-query-image` at fix commit `a44a26b287a328927c6d7bffa0a253b0d1a807dd`. Add the GitHub Actions workflow as one additional commit on this build-only branch. The workflow triggers only on pushes to that exact branch.
...
- Jellyfin server: the triggering build-branch commit, whose history must contain fix commit `a44a26b287a328927c6d7bffa0a253b0d1a807dd`
...
The official packaging Dockerfile intentionally retains its normal mutable transitive inputs, including Debian and Node base tags, the .NET 10 channel, and APT/Jellyfin FFmpeg packages. The workflow pins repository source and action revisions, records the resulting image digest, and does not claim byte-for-byte reproducibility for a later rebuild of the same commit.
...
The stable tag is the Unraid-friendly reference. The full-SHA tag is a commit-addressed audit reference, but a workflow rerun can overwrite it. Both tags must resolve to the workflow-recorded digest immediately after publication; only the digest is the immutable validation, rollback, and deployment identity.

Add OCI labels for:

- source repository;
- workflow/build revision `${GITHUB_SHA}`;
- fixed server commit `a44a26b287a328927c6d7bffa0a253b0d1a807dd`;
...
Updating server, web, packaging, or action revisions requires a reviewed workflow commit. Transitive runtime and OS packages continue to follow the pinned official packaging Dockerfile; the published digest, rather than a future rebuild, is the immutable artifact.
...
- Its commit remains `a44a26b287a328927c6d7bffa0a253b0d1a807dd`.
...
- stable and commit-addressed tags resolve to the workflow-recorded digest;
- package visibility is public.

Image checks on Unraid:

- anonymous `docker pull` succeeds for the commit-addressed tag and digest-qualified reference;
...
- Stable and commit-addressed GHCR tags point to the workflow-recorded digest.
